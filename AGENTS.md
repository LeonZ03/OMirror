...
- Current UI version: `1.8.4`; bundled runtime expected: scrcpy `4.1`.
...
- The persisted “仅熄手机屏幕” setting is hot-applied. Never add `--turn-screen-off` to the primary mirror or restart it for this feature. A separate control-only scrcpy process (`--no-video --no-audio --turn-screen-off`) owns the physical-screen-off state; helper failure must not close the primary mirror.
